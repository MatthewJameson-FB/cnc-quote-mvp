...
export default function QuoteWorkbenchEditor({ quote }: { quote: QuoteRecord }) {
  ...
  const [researchResult, setResearchResult] = useState<ResearchResult>({
    ...
    possible_part_numbers: quote.possible_part_numbers,
    useful_links: quote.useful_links || [],
    missing_requirements: quote.missing_requirements,
    suggested_next_action: quote.suggested_next_action,
    draft_message: null,
    research_status: quote.research_status,
    researched_at: quote.researched_at,
  });
  ...
  const runResearchAssistant = async () => {
    setResearchPending(true);
    setResearchError(null);

    try {
      const response = await fetch(`/api/admin/quotes/${quote.id}/research`, { method: "POST" });
      const payload = await response.json();

      if (!response.ok) {
        throw new Error(payload?.error || "Research assistant failed.");
      }

      setResearchResult({
        research_summary: payload.research_summary || null,
        possible_part_numbers: payload.possible_part_numbers || null,
        useful_links: Array.isArray(payload.useful_links) ? payload.useful_links : [],
        missing_requirements: payload.missing_requirements || null,
        suggested_next_action: payload.suggested_next_action || null,
        draft_message: payload.draft_message || null,
        research_status: payload.research_status || null,
        researched_at: payload.researched_at || null,
      });
      setGeneratedState("Research assistant completed. Review before taking action.");
    } catch (error) {
      setResearchError(error instanceof Error ? error.message : "Research assistant failed.");
    } finally {
    ...
    <form ref={formRef} action={formAction} className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)_minmax(340px,0.95fr)]">
      ...
      <section className="xl:col-span-3 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Research Assistant</h2>
            <p className="text-sm text-slate-600">Human-in-the-loop research helper. It prepares links and a draft only — no email is sent.</p>
          </div>
          <button
            type="button"
            onClick={runResearchAssistant}
          ...
          >
            {researchPending ? "Running…" : "Run research assistant"}
          </button>
        </div>

        {researchError ? <p className="mt-4 rounded-2xl bg-red-50 p-4 text-sm font-medium text-red-700">{researchError}</p> : null}

        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <div className="rounded-2xl bg-slate-50 p-4">
            <p className={labelClass()}>Research summary</p>
            <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">{researchResult.research_summary || "Not run yet."}</p>
          </div>
          <div className="rounded-2xl bg-slate-50 p-4">
            <p className={labelClass()}>Suggested next action</p>
            <p className="mt-2 text-sm font-semibold text-slate-900">{researchResult.suggested_next_action || "—"}</p>
            <p className="mt-2 text-xs text-slate-500">Status: {researchResult.research_status || "not_run"}{researchResult.researched_at ? ` · ${new Date(researchResult.researched_at).toLocaleString()}` : ""}</p>
          </div>
          <div className="rounded-2xl bg-slate-50 p-4">
            <p className={labelClass()}>Possible part numbers</p>
            <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">{researchResult.possible_part_numbers || "Run the assistant to prepare manual part-number checks."}</p>
          </div>
          <div className="rounded-2xl bg-slate-50 p-4">
            <p className={labelClass()}>Missing requirements</p>
            <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">{researchResult.missing_requirements || "Run the assistant to check missing requirements."}</p>
          </div>
        </div>

        <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-4">
          <p className={labelClass()}>Useful links</p>
          {researchResult.useful_links.length ? (
            <div className="mt-3 flex flex-wrap gap-2">
              {researchResult.useful_links.map((link) => (
                <a key={`${link.label}-${link.href}`} href={link.href} target="_blank" rel="noreferrer" className="rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-100">
                  {link.label}
                </a>
              ))}
            </div>
          ) : (
            <p className="mt-2 text-sm text-slate-500">Run the assistant to generate Google, image, eBay, part-number, STL, CAD, and forum links.</p>
          )}
        </div>

        <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className={labelClass()}>Draft message</p>
            <button
              type="button"
              onClick={() => researchResult.draft_message ? markMessage(researchResult.draft_message) : null}
              disabled={!researchResult.draft_message}
            ...
            >
              Use as customer message
            </button>
          </div>
          <p className="mt-2 whitespace-pre-wrap rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-700">{researchResult.draft_message || "Run the assistant to create an editable draft."}</p>
        </div>
      </section>
